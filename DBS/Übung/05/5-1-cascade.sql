-- Variante: ON UPDATE/DELETE CASCADE
...
create or replace function t_herstellercheck_func ()
  ...
  begin 
    ...
    if (TG_OP = 'DELETE') then
      delete from produkt where produkt.hnr = old.hnr;
      return old;
    elsif (TG_OP = 'UPDATE') then
      update produkt set hnr = new.hnr where produkt.hnr = old.hnr;
      return new;
    end if;
  end;
  $$ 
...
create trigger t_herstellercheck
  -- wichtig: hier brauchen wir einen after-Trigger, weil der geänderte Hersteller-Datensatz
  -- schon existieren muss, bevor die Änderung auf die betroffenen Produkt-Datensätze
  -- propagiert wird.
